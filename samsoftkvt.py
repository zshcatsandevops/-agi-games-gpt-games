...
# -------------------------
pygame.init()
pygame.display.set_caption("Toads vs Koopas — Tower Defense")
...
class Enemy(pygame.sprite.Sprite):
    ...
    def take_damage(self, dmg):
        if self.armor_hp > 0:
            ...
            self.draw_enemy(self.__class__.COLOR)
        if dmg > 0:
            self.hp -= dmg
        if self.hp <= 0:
            self.kill()
    ...
    def update(self, dt):
        ...
        if self.hp < self.max_hp:
            ...
            pygame.draw.rect(screen, GREEN, (bar_x, bar_y, int(bar_width * (self.hp / self.max_hp)), bar_height))
...
class GreenToad(Plant):
    ...
    def update(self, dt):
        super().update(dt)
        self.shoot_timer += dt
        lane_has_enemy = False
        for e in enemy_sprites:
            if e.row == self.row and e.rect.left > self.rect.right - 4:
                lane_has_enemy = True
                break
...
class BombToad(Plant):
    ...
    def update(self, dt):
        ...
        if self.fuse <= 0:
            ...
            boom = pygame.Surface((self.RADIUS*2, self.RADIUS*2), pygame.SRCALPHA)
            pygame.draw.circle(boom, (255, 80, 0, 110), (self.RADIUS, self.RADIUS), self.RADIUS)
            spr = pygame.sprite.Sprite()
            spr.surf = boom
            spr.rect = boom.get_rect(center=center)
            explosion_sprites.add(spr); all_sprites.add(spr)
            for e in enemy_sprites:
            ...
            if grid[self.row][self.col] is self:
                grid[self.row][self.col] = None
            self.kill()
            pygame.time.set_timer(USEREVENT + 99, 200, True)
...
class SeedCard:
    ...
    def draw(self, surf, can_afford, selected):
        ...
        icon_surf = pygame.Surface((self.rect.width-10, self.rect.height-20))
        icon_surf.fill((0, 0, 0, 0))
        
        temp_plant = self.cls(0, 0)
        temp_plant.rect = pygame.Rect(5, 5, self.rect.width-10, self.rect.height-20)
        temp_plant.surf = icon_surf
        temp_plant.draw_plant()
        surf.blit(icon_surf, (self.rect.x + 5, self.rect.y + 5))
        
        name = font.render(self.cls.NAME, True, BLACK)
        cost = font.render(f"{self.cls.COST}", True, BLACK)
        surf.blit(name, (self.rect.x + 4, self.rect.y + 4))
        surf.blit(cost, (self.rect.x + 4, self.rect.y + self.rect.height - 24))
    ...
    for proj in list(projectile_sprites):
        ...
        if hit:
            ...
            if proj.slow_factor > 0:
                hit.apply_slow(proj.slow_factor, proj.slow_duration)
            proj.kill()
...
def reset_game():
    global sun_count, grid, selected_card, shovel_selected, level
    sun_count = 50
    selected_card = None
    shovel_selected = False
    ...
    def update_loop():
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                spawn_sky_sun()
            elif event.type == USEREVENT + 99:
                for s in list(explosion_sprites):
                    s.kill()

        if state == STATE_PLAY:
            level.update(dt)
            for p in list(plant_sprites):
                p.update(dt)
            for s in list(sun_sprites):
                s.update(dt)
            for pr in list(projectile_sprites):
                pr.update(dt)
            for e in list(enemy_sprites):
                e.update(dt)
            for m in list(mower_sprites):
                m.update(dt)
